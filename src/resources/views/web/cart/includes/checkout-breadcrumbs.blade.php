@if (config("variation-cart.useBreadcrumbs"))
    ...
    <x-tt::breadcrumbs>
        ...
        <x-tt::breadcrumbs.item :url="route('web.cart')">{{ config("variation-cart.cartPageTitle") }}</x-tt::breadcrumbs.item>
        <x-tt::breadcrumbs.item>{{ config("variation-cart.checkoutPageTitle") }}</x-tt::breadcrumbs.item>
    </x-tt::breadcrumbs>
@endif
